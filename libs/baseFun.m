...
if order == 1
    %% P1
    phi = {@(x,y) 1-x-y, @(x,y) x, @(x,y) y};   % Basisfunktion
...
elseif order == 2
    ...
    numBaseFun = length(vertices);                  % Anzahl Basisfunktionem
    ...
    for i = 1:numBaseFun
        ...
        phi{i} = @(x,y) c(1) + c(2)*x + c(3)*y + c(4)*x.^2 +c(5)*x.*y + c(6)*y.^2; % Basisfunktion
