...
% TODO: wozu K_dir und F_dir, werden nicht verwendet, nichtmal
% zurueckgegeben beim funktionsausfruf
...
n_nodes = 2*size(p,2); % absolute Anzahl der Freiheitsgrade entspricht Anzahl an Knoten*2
...
for i=1:size(t,2) % Über die Elemente iterieren
    ...
    [MK,FK]=elasticMassLoad(phihat,f,B_affmap,d_affmap,detB_affmap,nBaseFun,quad_high); % lokale Massenmatrix und Lastvektor berechnen
...
% Knoten der Quadraturformel
xhat_quad = quad_high.knoten(:,1); 
yhat_quad = quad_high.knoten(:,2);
